...
local cmdCWatcher = hs.eventtap.new({ hs.eventtap.event.types.keyDown }, function(event)
	...
	-- Prüfen ob Cmd+C gedrückt wurde (C = keyCode 8)
	if keyCode == 8 and flags:containsExactly({ "cmd" }) then
		handleCmdC()
		return false -- Event weiterleiten (normaler Copy funktioniert weiter)
	end

	return false -- Alle anderen Events weiterleiten
end)
