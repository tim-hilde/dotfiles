...
`references/example.html` is a fully rendered page in this style — open it to see the tokens and the core dependency-free techniques applied correctly (comparison panels, semantic pills, a `<div>` bar chart, a token-themed interactive SVG, collapsible detail, copy-export with textarea fallback, and the opt-in dark toggle). It's the concrete "show" anchor; match it rather than re-deriving the look from the prose.
